...
animate_tracks <- function(df, facet = FALSE, facet_scales = 'fixed') {
  ...
  tryCatch({
   c('id', 'date', 'lat', 'lon') %in% colnames(df) %>%  all()
    ...
    stop(paste("Error: One or more input columns do not exist in the data frame.\n",
...
plot_tracks <- function(df, shapefile, facet = TRUE, facet_scales = 'fixed') {
  ...
  bathy <- getNOAA.bathy(lon1 = min_lon, lon2 = max_lon, lat1 = min_lat, lat2 = max_lat,
                         keep = TRUE,
...
plot_tracks_tmap <- function(df, shapefile, facet = TRUE) {
  ...
  australia_map <- tm_shape(world_map) +
    tm_polygons("region", name = "Australia", 
                palette = "grey90")
...
cleanWcLocationsData <- function(d) {
  ...
  # Rename columns
  d <- d %>% 
    rename(id = deploy_id, lat = latitude, lon = longitude, lc = quality,
